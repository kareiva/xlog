...
void on_awards_was_activate (GtkMenuItem *menuitem, gpointer user_data)
{
	if (!gtk_widget_get_visible(scorewindow))
		warningdialog (_("xlog - WAS award"),
			_("Please enable the scoring window"), "gtk-dialog-warning");
	if (preferences.awardswas == 0)
	{
		warningdialog (_("xlog - WAS award"),
                        _("You will need to enable WAS scoring from the \"windows and dialogs\" dialog"), "gtk-dialog-warning");
		return;
	}

	GtkWidget *dialog = gtk_dialog_new_with_buttons (_("xlog - WAS award"),
	...
	/* 2 extra columns, state and 'all bands */
	...
	for (i = 0; i < MAX_STATES; i++)
	{
		...
		gtk_list_store_set (GTK_LIST_STORE (model), &iter, 0, enum_to_state(i), -1);
		for (j = 0; j < MAX_BANDS; j++)
		{
			...
			{
				gchar *str = g_strdup_printf ("%d", was_w[i][j]);
