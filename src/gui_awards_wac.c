...
void on_awards_wac_activate (GtkMenuItem *menuitem, gpointer user_data)
{
	if (!gtk_widget_get_visible(scorewindow))
		warningdialog (_("xlog - WAC award"),
			_("Please enable the scoring window"), "gtk-dialog-warning");
	if (preferences.awardswac == 0)
	{
		warningdialog (_("xlog - WAC award"),
                        _("You will need to enable WAC scoring from the \"windows and dialogs\" dialog"), "gtk-dialog-warning");
		return;
	}

	GtkWidget *dialog = gtk_dialog_new_with_buttons (_("xlog - WAC award"),
	...
	/* 2 extra columns, continent and 'all bands */
	...
	for (i = 0; i < MAX_CONTINENTS; i++)
	{
		...
		gtk_list_store_set (GTK_LIST_STORE (model), &iter, 0, enum_to_cont(i), -1);
